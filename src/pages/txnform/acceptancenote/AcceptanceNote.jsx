...
const AcceptanceNote = () => {
  ...
  const onFinish = async (values) => {
    try {
      ...
      const apiUrl = 'https://sai-services.azurewebsites.net/sai-inv-mgmt/saveAcceptanceNote';
      ...
      if (response.status === 200 && response.data && response.data.responseStatus && response.data.responseStatus.message === 'Success') {
        ...
        setSuccessMessage(`Acceptance Note : ${processId}, Process Type: ${processType}, Sub Process ID: ${subProcessId}`);
        showModal();
        message.success(`Acceptance Note  successfully! Process ID: ${processId}, Process Type: ${processType}, Sub Process ID: ${subProcessId}`);
  ...
  return (
    ...
    <div className="goods-receive-note-form-container">
      ...
      <Form onFinish={onFinish} className="goods-receive-note-form" onValuesChange={handleValuesChange} layout="vertical">
              ...
              {fields.map(({ key, name, ...restField }, index) => (
                <div key={key} style={{ marginBottom: 16, border: '1px solid #d9d9d9', padding: 16, borderRadius: 4 }}>
                  <Row gutter={24}>
                    ...
                    <Col span={6}>
                      <Form.Item {...restField} label="ACCEPTED QUANTITY" name={[name, 'quantity']}>
        ...
        <Modal title="Acceptance Note saved successfully" visible={isModalOpen} onOk={handleOk} >
